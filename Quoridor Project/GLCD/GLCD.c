...
void LCD_DrawRect(uint16_t Xpos, uint16_t Ypos, uint16_t heigth, uint16_t width, uint16_t bkColor) {
	...
	for (i = 0; i < heigth; i++) {
		...
		LCD_WriteIndex(0x0022);	// Sets writing into GRAM
		
		for (j = 0; j < width; j++) {
			LCD_WriteData(bkColor);	// Starting from last SetCursor, this writes data into GRAM (writes columns)
		}
	}
}

void LCD_DrawGrid(void){
		const uint16_t cellSize = 28;
    const uint16_t spacing = 7;
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            uint8_t startX = j * (cellSize + spacing) + 1;
            uint8_t startY = i * (cellSize + spacing) + 1;
            LCD_DrawRect((uint16_t) startX, (uint16_t) startY, cellSize, cellSize, Black);
					  LCD_DrawRect((uint16_t) startX+3, (uint16_t) startY+3, cellSize-6, cellSize-6, White);
        }
    }
}

void LCD_DrawPlayer(uint16_t Xpos, uint16_t Ypos, uint16_t color) 
{
		// Calculate the center of the circle (convert from grid to LCD)
    int circleCenterX, circleCenterY;
		circleCenterX = 1 + (CELL_SIZE+SPACING)*Xpos + 14;		
		circleCenterY = 1 + (CELL_SIZE+SPACING)*Ypos + 14;
    LCD_DrawFullCircle(circleCenterX, circleCenterY, PLAYER_SIZE, color);
}
...
void LCD_DrawFullCircle(uint16_t Xpos, uint16_t Ypos, uint16_t radius, uint16_t color) {
    ...
    while (y >= x) {
        // Draw points for each octant of the circle
        LCD_DrawLine(Xpos - x, Ypos + y, Xpos + x, Ypos + y, color);
        LCD_DrawLine(Xpos - y, Ypos + x, Xpos + y, Ypos + x, color);
        LCD_DrawLine(Xpos - y, Ypos - x, Xpos + y, Ypos - x, color);
        LCD_DrawLine(Xpos - x, Ypos - y, Xpos + x, Ypos - y, color);

        x++;

        // Check the decision parameter to decide the next point
        if (d > 0) {
            y--;
            d = d + 4 * (x - y) + 10;
        } else {
...
void LCD_DrawWall(uint16_t Xpos, uint16_t Ypos, uint16_t color){
	uint16_t startX, startY;
	if (orientation == 0) //verticale
	{
		startX = 1 + Xpos * CELL_SIZE + (Xpos-1) * SPACING;	 
		startY = 1 + Ypos * (CELL_SIZE + SPACING);
		LCD_DrawRect(startX, startY, WALL_LENGHT, WALL_WIDTH, color);
	}
	else 
	{
		startX = 1 + Xpos * (CELL_SIZE + SPACING);
		startY = 1 + Ypos* CELL_SIZE + (Ypos-1) * SPACING;
		LCD_DrawRect(startX, startY, WALL_WIDTH, WALL_LENGHT, color);
	}
}

void LCD_DrawInterface(){
	LCD_DrawRect(LCD_P1_START_X, LCD_P1_START_Y, LCD_P1_LENGTH, LCD_P1_WIDTH, Black);
	LCD_DrawRect(LCD_P1_START_X+3, LCD_P1_START_Y+3, LCD_P1_LENGTH-6, LCD_P1_WIDTH-6, White);
	GUI_Text(20, 280, (uint8_t *)"P1 Wall", Black, PLY0_COLOR);
	GUI_Text(43, 295, (uint8_t *) "8", Black, White);
	
	LCD_DrawRect(LCD_TIMER_START_X, LCD_TIMER_START_Y, LCD_TIMER_LENGTH, LCD_TIMER_WIDTH, Black);	
	LCD_DrawRect(LCD_TIMER_START_X+3, LCD_TIMER_START_Y+3, LCD_TIMER_LENGTH-6, LCD_TIMER_WIDTH-6, White); 
	
	LCD_DrawRect(LCD_P2_START_X, LCD_P2_START_Y, LCD_P2_LENGTH, LCD_P2_WIDTH, Black);
	LCD_DrawRect(LCD_P2_START_X+3, LCD_P2_START_Y+3, LCD_P2_LENGTH-6, LCD_P2_WIDTH-6, White);
	GUI_Text(170, 280, (uint8_t *)"P2 Wall", Black, PLY1_COLOR);
	GUI_Text(193, 295, (uint8_t *) "8", Black, White);
}
